...
//dashboard
$(document).ready(function(){
  $('#dashboard_implementation').load('http://localhost:8080/group39_fitbot_war_exploded/Virtual%20Member/Dashboard/Dashboard.html #dashboard_middle',function(responseTxt, statusTxt, xhr){
  if(statusTxt == "error")
      alert("Error: " + xhr.status + ": " + xhr.statusText);
  });
  load[0] += 1;
...
$(document).ready(function(){
  ...
  $('#phy_mem_workout').click(function(){
    if(load[4] == 0){
      ...
      $(sideBar_links_variable).load('http://localhost:8080/group39_fitbot_war_exploded/Virtual%20Member/Workout_Plans/Workout_Plan.html #workout_plan_physical',function(responseTxt, statusTxt, xhr){
        if(statusTxt == "error")
          alert("Error: " + xhr.status + ": " + xhr.statusText);
        // alert(responseTxt);
        // alert(statusTxt);
  ...
  $('#phy_mem_diet_plan').click(function(){
    if(load[5] == 0){
      ...
      $(sideBar_links_variable).load('http://localhost:8080/group39_fitbot_war_exploded/Virtual%20Member/Diet_Plans/Diet_Plans.html #Diet_Plans_physical',function(responseTxt, statusTxt, xhr){
...
$(document).ready(function(){
  ...
  $.ajax({
    ...
    $('#dashboard_mambership_first_text').append(
        '<span class="dashboard_mambership_second_text"><b>'+'Virtual Member'+'</b></span>'
    );
  ...
  $.ajax({
    ...
    if(paymentId == 0){
      $('#payment_date_details').append(
          '<span class="previous_payment_date">Next Payment Date</span><br>'+
          // '<span class="previous_payment_date_format">'+x.new_expire_date["year"]+"-"+x.new_expire_date["month"]+"-"+x.new_expire_date["day"]+'</span><br>'+
          '<span class="previous_payment_date_details">No Last payment</span>'
      );
    }
  }).fail(function(a,b,err){
    alert("Error");
